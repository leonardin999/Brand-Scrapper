...
def open_page(url):
    ...
    browser.wait_until_element_is_visible("css=div.page-description",timeout=45)
...
def get_brand_infos(id,directory="data"):
    try:
        ...
        filename = brand_name.replace(".","")
        description = browser.get_text("css=div.item-info")
        is_enough = False
        products = []
        while not is_enough:
            browser.wait_until_element_is_visible("css=div.product-list",timeout=45)
            try:
...
if __name__ == "__main__":
    url = "https://www.ontrium.com/en/usa/brands-B"
    # create saving folder:
    directory = "data/brands-B"
